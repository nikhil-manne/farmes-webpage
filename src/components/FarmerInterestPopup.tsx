...
export const FarmerInterestPopup = () => {
  ...
  useEffect(() => {
    ...
    // Show the floating icon after 2 seconds
    ...
    <>
      ...
      {!hasResponded && (
        <button
          ...
          className={cn(
            "fixed bottom-24 right-6 z-50 flex items-center justify-center w-14 h-14 rounded-full bg-green-600 text-white shadow-2xl transition-all hover:scale-110 active:scale-95 group animate-in slide-in-from-right-10 duration-500",
            isOpen ? "scale-0 opacity-0" : "scale-100 opacity-100"
          )}
        >
          <Heart className="w-6 h-6 fill-white group-hover:animate-pulse" />
          <span className="absolute right-full mr-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-xl border border-slate-100 dark:border-slate-800 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity">
            Help us grow! 🌿
          </span>
        </button>
      )}
            ...
            <div className="space-y-8">
              <div className="space-y-4 text-center">
                ...
                <p className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
                  Please let us know if you are interested - this could help us for sourcing directly with farmers.
                </p>
              </div>
